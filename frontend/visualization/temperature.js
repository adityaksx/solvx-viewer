...
export class TemperatureVisualizer {
    ...
    applyVolumeColors() {
        ...
        this.water.recolorCells(c => {
            ...
            const color = sampleColor('temperature', val, this.range.lo, this.range.hi);
    ...
    updateSliceTexture(depthM, mode = 'slice') {
        ...
        if (mode === 'vertical') {
            ...
            import('./verticalSlice.js').then(module => {
                const texture = module.createVerticalSliceTexture(this.currentData, 'temperature', this.range.lo, this.range.hi);
        ...
        const texture = createDataTexture2D(tempFakeArray, 'temperature', this.range.lo, this.range.hi);
